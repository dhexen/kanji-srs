    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Pendientes hoy', val: pending, color: 'text-amber-600' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
          <h3 className="font-bold text-slate-900">☁️ Cuenta y Sincronización</h3>
          ...
          {state.user ? (
            <div className="space-y-3">
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-bold text-emerald-800">Sincronización activa</span>
                </div>
                <p className="text-xs text-emerald-600">{state.user.email}</p>
                <p className="text-xs text-emerald-500 mt-1">Tu progreso se guarda automáticamente en la nube tras cada respuesta.</p>
              </div>
              {state.syncing && (
                <div className="text-xs text-indigo-500 flex items-center gap-2 animate-pulse">
                  <span>↕</span> Sincronizando...
                </div>
              )}
              <button
                onClick={() => handleAuth(logout, 'Sesión cerrada')}
            ...
            <div className="space-y-3">
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
                ⚠️ Sin sesión activa. Tu progreso solo se guarda en este navegador.
              </div>
              <input type="text" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="Email" autoComplete="off" data-lpignore="true"
                className="w-full px-3 py-2 border rounded-lg text-sm" />
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="Contraseña" autoComplete="new-password" data-lpignore="true"
                className="w-full px-3 py-2 border rounded-lg text-sm" />
              <div className="flex gap-2">
                <button disabled={authLoading}
                  onClick={() => handleAuth(() => login(email, password), '¡Sesión iniciada!')}
                ...
                <button disabled={authLoading}
                  onClick={() => handleAuth(() => signup(email, password), '¡Cuenta creada! Revisa tu email.')}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4 flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-slate-900">💾 Copia de Seguridad</h3>
            <p className="text-slate-500 text-xs mt-1">Exporta o importa un archivo JSON con tu progreso.</p>
            <div className="flex gap-3 mt-4">
              <button onClick={exportJSON}
                className="flex-1 py-2.5 border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold rounded-xl text-sm transition flex items-center justify-center gap-1">
                📤 Exportar
              </button>
          ...
          <button onClick={resetAll}
            className="w-full py-3 border border-red-200 bg-red-50 hover:bg-red-100 text-red-600 font-bold rounded-xl text-sm transition">
              ...
              {state.db.map(item => (
                <tr key={item.jp} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  {(Object.keys(MODE_CONFIG) as ReviewMode[]).map(mode => {
                    ...
                    return (
                      <td key={mode} className="py-3 px-3 text-center">
                        <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${getSrsClass(level, item.status, due)}`}
